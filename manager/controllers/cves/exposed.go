...
func (c *Controller) BuildExposedClustersQuery(cveName string, accountID int64, clusterIDs []string) *gorm.DB {
	query := c.Conn.Table("cluster").
		Select(`cluster.uuid,
				COALESCE(cluster.display_name, cluster.uuid),
				COALESCE(cluster.status, 'N/A'),
				COALESCE(cluster.type, 'N/A'),
				COALESCE(cluster.version, 'N/A'),
				COALESCE(cluster.provider, 'N/A'),
				cluster.last_seen,
